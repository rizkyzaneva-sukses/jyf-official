...
export function PurchaseOrdersTab() {
  ...
  const handleToggleClose = async (po: any, action: 'close' | 'reopen') => {
    ...
    setClosingId(po.id)
    try {
      const res  = await fetch(`/api/purchase-orders/${po.id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ action }) })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error)
      toast({ title: json.data.message, type:'success' })
      qc.invalidateQueries({ queryKey:['purchase-orders'] })
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {(isOwner || isFinance) && (po.status === 'OPEN' || po.status === 'PARTIAL') && (
                          <button onClick={() => handleToggleClose(po, 'close')} disabled={closingId===po.id}
                            className="p-1.5 rounded bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-zinc-200 disabled:opacity-40" title="Tutup PO">
                            <Lock size={13}/>
                          </button>
                        )}
                        {(isOwner || isFinance) && po.status === 'CLOSED' && (
                          <button onClick={() => handleToggleClose(po, 'reopen')} disabled={closingId===po.id}
                            className="p-1.5 rounded bg-zinc-800 hover:bg-emerald-900/40 text-zinc-400 hover:text-emerald-400 disabled:opacity-40" title="Buka Kembali">
                            <Unlock size={13}/>
                          </button>
                        )}
