...
export function ScanTab() {
  ...
  const handleCommit = async () => {
    ...
    try {
      const batchRes  = await fetch('/api/scan', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ direction: tab.direction, reason: tab.reason, vendorId: activeTab === 'masuk' ? vendorId || undefined : undefined, vendorName: activeTab === 'masuk' && vendorId ? (vendorsData || []).find((v: any) => v.id === vendorId)?.namaVendor : undefined, itemsWithDetails: items.map(i => ({ sku: i.sku, qty: i.qty, trxDate: i.trxDate, supplierName: i.supplierName, note: i.note })) }) })
  ...
  const switchTab = (key: TabKey) => {
    ...
    setActiveTab(key); setItems([]); setLookupError(''); setVendorId('')
